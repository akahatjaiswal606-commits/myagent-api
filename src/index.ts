...
const checkPlan = async (req: any, res: any, next: any) => {
  try {
    ...
    const limits: Record<string, { messages: number; agents: number }> = {
      free: { messages: 50, agents: 1 },
      pro: { messages: 1000, agents: 10 },
      enterprise: { messages: 99999, agents: 100 },
    };

    const limit = limits[plan] || limits["free"];

    if (monthlyMessages >= limit.messages) {
